...
const InputKeySlider = ({ route }) => {
  //declare state variables
  console.log("Slides: ", route.params.path, route.params.files)
  ...
  const done =() =>{
    ...
    if(key.trim()===""){
        ...
        const Slides = []
        for( let i=0; i<route.params.files.length; i++){
            Slides.push({hash: route.params.files[i], privKey: key})
        }
        console.log("All Slides: ", Slides)
        navigation.navigate(route.params.path, {Slides: Slides})
    }
