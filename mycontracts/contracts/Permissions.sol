...
contract Permissions{
   ...
   struct File {
    ...
    function getAll(uint n) public view returns (bool[] memory){
        ...
        for (uint i = 0; i < files[n].addressNum; i++) {
            ret[i] = files[n].permission[files[n].addressNumbering[files[n].addressNum]];
        }
        return ret;
    }
    ...
    function getPermissionFilesbyDoctor(address doctorId)public view returns (string[] memory){
        uint size=0;
    ...
    function getFilesHashByPermissionedHash(string memory pHash,address doctorId)public view returns (string memory){
    ...
    function getAllFilesbyPatient(address patientId)public view returns (string[] memory){
        uint size=0;
        for(uint i=0;i<fileCount;i++){
   ...
   function grantPermission(uint n,address docID,address origin, string memory pHash) external { //Origin is the caller of the function
        require(files[n].patientID==origin); //Caller of function should be the owner of the file
        require(stringsEquals(myRoles.returnRole(origin),"Patient"),"Not authorized"); //Owner of the file must be a patient
    ...
    function revokePermission(uint n,address docID,address origin) external {
        ...
        require(stringsEquals(myRoles.returnRole(origin),"Patient"),"Not authorized");
