...
const SignupDoctor = () => {
  ...
  //creating a new doctor in database and smart contracts
  const signup = () => {
    if (name.trim() === '') {
      ...
      const response = fetch(`${HTTP_CLIENT_URL}/doctor/create`, {
        method: 'POST',
        headers: {
        ...
        if (d.success) {
          ...
          const addressid = d.doctor.addressid

          fetch(`${HTTP_CLIENT_URL}/contracts/addDoctor`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            ...
            if (d1.success) {
              ...
              try {
                ...
                navigation.navigate("Key", { paramKey: d.key, role: 'doctor' });
              } catch (error) {
                setResult(error);
                setModalVisible(true);
                // Error saving data
  ...
  //on pressing okay close the model
  const ok = () => {
